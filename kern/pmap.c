...
boot_alloc(uint32_t n)
{
  ...
  if(n > 0)
  {
      ...
      if((uint32_t)nextfree - KERNBASE >  4*PGSIZE*PGSIZE)
...
{ 
	...
	// free pages!
	size_t i=0;
  void* kend = boot_alloc(0);
  pages[i].pp_ref = 1;
  for(i = 1; i<npages; ++i)
  {
    if(page2pa(&pages[i]) >= IOPHYSMEM && page2pa(&pages[i])<EXTPHYSMEM)
      pages[i].pp_ref=1;
    else if(page2pa (&pages[i])>=EXTPHYSMEM && page2pa(&pages[i]) <PADDR(kend))
      pages[i].pp_ref=1;
    else
    {
      pages[i].pp_ref=0;
...
page_free(struct PageInfo *pp)
{
   ...
   if(pp->pp_ref != 0 || pp->pp_link != NULL)
     panic("pp_link not a NULL, or pp_ref is nonzero.");
